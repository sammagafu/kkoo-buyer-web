...
const showcaseImages: Record<string, string> = {
  eats: foodImage,
  grocery: groceriesImage,
  marketplace: shopImage,
  send: rideImage,
  ride: rideImage,
  courier: rideImage,
  restaurants: foodImage,
  hotels: heroImage,
  booking: heroImage,
  vouchers: shopImage,
  'share-earn': heroImage,
  community: catImage,
}

export const premiumShowcaseItems: PremiumShowcaseItem[] = superAppServiceGroups.consumer
  .flatMap((g) => g.items)
  .filter((item): item is typeof item & { to: RouteLocationRaw } => Boolean(item.to))
  .map((item) => ({
    key: item.key,
    icon: item.icon,
    image: showcaseImages[item.key] ?? heroImage,
    titleKey: item.labelKey,
    descKey: item.descKey,
    altKey: item.labelKey,
    to: item.to,
  }))
...
export const premiumServiceCards: PremiumServiceCard[] = superAppServiceGroups.consumer
  .flatMap((g) => g.items)
  .slice(0, 6)
  .map((item) => ({
  key: item.key,
