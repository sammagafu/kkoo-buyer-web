...
function onSearchSubmit() {
  ...
  if (isAdmin) {
    window.open(`${adminWebPath('/admin/catalog/products')}?search=${encodeURIComponent(q)}`, '_blank')
  } else if (auth.isSeller) {
    window.open(`${bizWebPath('/seller/products')}?search=${encodeURIComponent(q)}`, '_blank')
  } else {
    router.push({ name: 'buyer.marketplace', query: { search: q } });
  }
}
...
function notificationOrderLink(n: { data?: Record<string, unknown> }) {
  ...
  if (id !== undefined && (auth.role === ROLES.ADMIN || auth.role === ROLES.STAFF)) return adminWebPath(`/admin/orders/${id}`);
