...
/** Public + auth. API.md: GET /promotions/ (no auth), POST /promotions/apply/ (auth), POST /promotions/apply-ride/ (ride/delivery). */
export const promotionsApi = {
  list() {
    return client.get<{ results: unknown[] }>('/promotions/')
  },
  apply(promo_code: string, order_amount: number) {
    return client.post<{
      valid: boolean
      code?: string
      ...
      order_amount?: number
      final_amount?: number
    }>('/promotions/apply/', { promo_code, order_amount })
  },
  /** Ride/delivery promo: validate code against delivery_fee; use before setting delivery_fee on order. */
  applyRide(code: string, delivery_fee: number) {
    return client.post<{
      valid: boolean
      code?: string
      discount_amount?: number
      delivery_fee?: number
      final_fee?: number
    }>('/promotions/apply-ride/', { code, delivery_fee })
  },
}
